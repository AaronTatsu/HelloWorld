...
public class RecyclerTouchHelper extends ItemTouchHelper.SimpleCallback {
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int ActionState, boolean isCurrentlyActive){
        ...
        int iconMarginTop =  itemView.getTop() + (itemView.getHeight() - icon.getIntrinsicHeight()) /2;
        int iconBottom = iconMarginTop + icon.getIntrinsicHeight();

        if(dX>0){
            //swipe to right
            int iconLeft = itemView.getLeft() + iconMargin;
            int iconRight = itemView.getLeft() + iconMargin + icon.getIntrinsicWidth();
            icon.setBounds(iconLeft, iconMarginTop, iconRight, iconBottom);
